...
class Definitions {
  ...
  private def copyMethod(fromSymbol: TermSymbol, toCls: ClassSymbol, toName: TermName): TermSymbol =
    enterMethod(toCls, toName, fromSymbol.info, fromSymbol.flags)
  ...
  def AnyValType = AnyValClass.typeRef
    ...
    lazy val Any_eqPrecise = copyMethod(Any_==, AnyClass, NameKinds.PrecisePrimName(nme.EQ))
    lazy val Any_nePrecise = copyMethod(Any_!=, AnyClass, NameKinds.PrecisePrimName(nme.NE))
    lazy val Any_equalsPrecise = copyMethod(Any_equals, AnyClass, NameKinds.PrecisePrimName(nme.equals_))
  ...
  lazy val PreciseAnnotType = ctx.requiredClassRef("scala.annotation.precise")
  def PreciseAnnot(implicit ctx: Context) = PreciseAnnotType.symbol.asClass
...
//    valueTypeNameToJavaType(vcls.name) = jtype
  ...
  def impreciseAnyEqSymbol(sym: Symbol)(implicit ctx: Context): Symbol =
    if (sym eq Any_eqPrecise) Any_==
    else if (sym eq Any_nePrecise) Any_!=
    else sym
