...
class Definitions {
  ...
  def AnyValType = AnyValClass.typeRef
    ...
    lazy val Any_asInstanceOf = enterT1ParameterlessMethod(AnyClass, nme.asInstanceOf_, _.paramRefs(0), Final | Stable)
...
//    javaTypeToValueTypeRef(jtype) = vcls
  ...
  lazy val iteMethod = newSymbol(PTyperPackageClass, nme.ITE, Method | Stable,
    MethodType(List(defn.BooleanType, defn.AnyType, defn.AnyType), defn.NothingType))
