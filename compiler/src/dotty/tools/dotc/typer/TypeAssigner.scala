...
trait TypeAssigner {
  ...
  def assignType(tree: untpd.PredicateTypeTree, subjectVd: ValDef, pred: Tree)(implicit ctx: Context) = {
    val tp =
      if (pred.tpe.isStable) PredicateRefinedType(subjectVd, pred)
      else errorType(s"Predicate must be stable.", pred.pos)
    tree.withType(tp)
  }
