...
class Extractor(implicit val ctx: Context) extends ASTExtractors {

  import ExpressionExtractors._

  protected val state = ExtractionState(Bijection(), mutable.Set())
  ...
  case class ExtractionState(symsToIds: Bijection[Symbol, Identifier], unboundIds: mutable.Set[Identifier]) {
    def registerSym(sym: Symbol, tpe: LeonType = Untyped) = {
  ...
  abstract class Extraction[From, To] {
    ...
    protected def extractTree(tree: DottyTree)(implicit state: ExtractionState, env: TemplateEnv): Try[LeonExpr] =
      tree match {
        ...
        case ExEquals(lhs, rhs) =>
          extractEquals(tree, lhs, rhs)

        case ExNotEquals(lhs, rhs) =>
          for (expr <- extractEquals(tree, lhs, rhs))
            yield Not(expr)
    ...
    @inline
    protected def extractEquals(tree0: DottyTree, lhs: DottyTree, rhs: DottyTree)
                               (implicit state: ExtractionState, env: TemplateEnv): Try[LeonExpr] =
      for (xLhs <- extractTree(lhs); xRhs <- extractTree(rhs))
        yield (xLhs, xRhs) match {
    ...
    protected def maybeExtractPrimitiveCall(tree0: DottyTree, recv: LeonExpr, sym: Symbol, args: List[LeonExpr])
                                           (implicit env: TemplateEnv): Try[LeonExpr] =
...
//          failExpr(tree0, "Not a primitive a primitive call (or not implemented yet)")
    ...
    override protected def extractIdentifier(tree0: DottyTree, sym: Symbol)
      ...
      tree0 match {
        case dotc.ast.Trees.Ident(dotc.liquidtyper.SubjectVarName) =>
          ...
          Variable(Extractor.subjectVarId(leonType)).setPos(tree0.pos)
  ...
  private def simpleLeonTypeToQType(leonType: LeonType, qualifier: Qualifier): QType = leonType match {
    ...
    case UninterpretedLeonType(original)                => QType.UninterpretedType(original)
    case _                                              =>
  ...
  private def preciseQType(env: TemplateEnv, tpe: DottyType, rhsExpr: LeonExpr): QType = {
    ...
    val preciseExpr = Equals(Variable(Extractor.subjectVarId(expectedTpe)), rhsExpr)
  ...
  def registerSymbol(sym: Symbol, tpe: LeonType): Identifier =
    state.registerSym(sym, tpe)

  def lookupSymbol(sym: Symbol): Identifier =
    state.symsToIds.toB(sym)

  def unboundIds: Set[Identifier] =
    state.unboundIds.toSet
...
object Extractor {
  ...
  private val _subjectVarId: Map[LeonType, Identifier] =
    Seq[LeonType](BooleanType, Int32Type).map { leonTp =>
      (leonTp, FreshIdentifier(SubjectVarName.toString, leonTp))
    }.toMap

  def subjectVarIds: Set[Identifier] = _subjectVarId.values.toSet

  def subjectVarId(leonTp: LeonType): Identifier = _subjectVarId(leonTp)
